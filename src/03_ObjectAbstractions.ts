...
class IncomingSheet extends Sheet {
  ...
  capOffAndReturnTotal(totalRow : RowNumber = this.getTotalRow()) : number {
    const traversable = this.sheet.getRange(totalRow, Column.D);
  ...
  addFundsEntry(e: ExpenseEntry) : void {
    if ( e.isIncoming ) {
      ...
      this.setCellValue(newRow, Column.C, e.entry);
      this.setCellValue(newRow, Column.D, `${e.cost}`);
      this.setCellValue(newRow, Column.E, e.tag);
    }
  }
}